...
RSpec.describe User, type: :model do
  describe 'validations' do
    context 'when name and post_counter are present' do
      it 'is valid' do
        user = User.new(name: 'Supreeti', post_counter: 0)
    ...
    context 'when name is too long' do
      it 'is invalid' do
        user = User.new(name: 'S' * 51, post_counter: 5)
        expect(user).to_not be_valid
      end
    end
    ...
    context 'when post_counter is negative' do
      it 'is invalid' do
        user = User.new(name: 'Supreeti', post_counter: -1)
        expect(user).to_not be_valid
      end
    end
  ...
  describe 'recent_posts' do
    it 'returns the most recent posts' do
      user = User.create(name: 'Supreeti', post_counter: 0)
